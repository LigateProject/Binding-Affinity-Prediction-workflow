...
#SBATCH --job-name=BAPGROMACSLiGenIntegration
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -G 0
#SBATCH --mem=8GB
...
#tbr1121PDBids=(2hl4 2hmu 2hmv 2hnc 2hnx 2hoc 2hs1 2hu6 2hxm 2hkf)
...
#tbr1851PDBids=(3ekw 3ekx 3el1 3el4 3el5 3el9 3elc 3eqr 3evd 3ery)
...
#tbr4221PDBids=(5ltn 5lud 5lvd 5lvl 5lvq 5lvr 5lwd 5lwm 5lyr 5lyn)
...
# work in /scratch not to overwhelm file system
cd /scratch
if [ -d ${PDBid} ]
then
rm -r ${PDBid}
echo "For ${PDBid}, the remainders of a previous unsuccessful script execution had to be removed."
fi
cp -r ${OUTPUT_PATH}/${PDBid} .
cd ${PDBid}
...
# generate topology and .gro file for ligand
obabel "${INPUT_PATH}/${PDBid}/${PDBid}_ligand.mol2" -O "${PDBid}.mol2"
...
content=$(head -n ${i} ${PDBid}.mol2 | tail -n 1 | rev | cut -c1-18 | rev | cut -c1-3)
### check number of residues in ligand (blind spot: a chain of the same type of residue is considered one residue)
if (( ${residueCount} < 2 ))
then
...
while IFS= read -r line; do
    ...
    echo "$line"
done < ${PDBid}.mol2 > tmp_mol2
cat tmp_mol2 > ${PDBid}.mol2
rm tmp_mol2
fi
...
obabel "${PDBid}.mol2" -O "${PDBid}_ligand.pdb"

# sometimes, OpenBabel uses atom names that are incompatible with GROMACS
sed -i ${PDBid}_ligand.pdb -e "s/O1 /OXT/g"
sed -i ${PDBid}_ligand.pdb -e "s/O30/OXT/g"
sed -i ${PDBid}_ligand.pdb -e "s/O48/OXT/g"
sed -i ${PDBid}_ligand.pdb -e "s/O65/OXT/g"
sed -i ${PDBid}_ligand.pdb -e "s/OE /OXT/g"

# not happy about using -ignh
set +e
gmx pdb2gmx -f ${PDBid}_ligand.pdb -renum -ignh -p ligand.top -i posre_Ligand.itp -o ligand.gro <<-eof
6
1
eof
set -e
...
if ! [ -s ligand.top ]
then
echo "For ${PDBid}, pdb2gmx could not generate a topology for the peptide, DNA or RNA ligand. Deleting folder as this complex should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
rm -r ${OUTPUT_PATH}/${PDBid}
continue
...
if $(tail -${tailNumber} ${PATH_TO_SLURM}/slurm-${SLURM_JOB_ID}.out | grep -q "Long Bond")
then
echo "For ${PDBid}, the peptide, DNA or RNA ligand has long bonds. Deleting folder as this complex should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
rm -r ${OUTPUT_PATH}/${PDBid}
continue
...
gmx editconf -f full.gro -o full.pdb

# add TER signal at the end of all protein chains
OC2=$(grep -c "OC2" full.pdb)

for i in $(seq 1 ${OC2}); do
lineNumber=$(grep -n "OC2" full.pdb | head -${i} | tail -1 | cut -d ":" -f 1)
lineNumber2=$(wc -l full.pdb | cut -d " " -f 1)
head -${lineNumber} full.pdb >> full_final.pdb
echo "TER" >> full_final.pdb
tail -$(( ${lineNumber2} - ${lineNumber} )) full.pdb >> full_final.pdb
mv full_final.pdb full.pdb
done

lineNumber=$(grep -n "HOH" full.pdb | head -1 | cut -d ":" -f 1)
lineNumber=$((${lineNumber} - 1))
if ! $(head -${lineNumber} full.pdb | tail -1 | grep -q "TER")
then
lineNumber2=$(wc -l full.pdb | cut -d " " -f 1)
head -${lineNumber} full.pdb >> full_final.pdb
echo "TER" >> full_final.pdb
...
# clean up
rm ${PDBid}_ligand.pdb ${PDBid}.mol2 ligand.gro conf.gro ligand.top topol* posre*itp full.gro
...
sed -i full.pdb -e "s/10ZN/  ZN/g"
sed -i full.pdb -e "s/11ZN/  ZN/g"
sed -i full.pdb -e "s/12ZN/  ZN/g"
sed -i full.pdb -e "s/0ZN/ ZN/g"
sed -i full.pdb -e "s/1ZN/ ZN/g"
sed -i full.pdb -e "s/2ZN/ ZN/g"
sed -i full.pdb -e "s/3ZN/ ZN/g"
sed -i full.pdb -e "s/4ZN/ ZN/g"
sed -i full.pdb -e "s/5ZN/ ZN/g"
sed -i full.pdb -e "s/6ZN/ ZN/g"
sed -i full.pdb -e "s/7ZN/ ZN/g"
sed -i full.pdb -e "s/8ZN/ ZN/g"
sed -i full.pdb -e "s/9ZN/ ZN/g"
sed -i full.pdb -e "s/8CA/ CA/g"
sed -i full.pdb -e "s/4NA/ NA/g"
sed -i full.pdb -e "s/6NA/ NA/g"
sed -i full.pdb -e "s/3K / K /g"
sed -i full.pdb -e "s/4K / K /g"
sed -i full.pdb -e "s/2MG/ MG/g"
sed -i full.pdb -e "s/3MG/ MG/g"
...
then
numberOfChainsToBeMerged=$(grep -c "TER" full.pdb)
numberOfChainsToBeMerged=$(( ${numberOfChainsToBeMerged} - 3 ))
touch createTopology.sh
echo "gmx pdb2gmx -f full.pdb -p topol_amber.top -i posre.itp -o full.gro -chainsep ter -merge interactive <<-eof" >> createTopology.sh
echo "6" >> createTopology.sh
echo "1" >> createTopology.sh
for i in $(seq 1 ${numberOfChainsToBeMerged}); do
...
gmx pdb2gmx -f full.pdb -p topol_amber.top -i posre.itp -o full.gro -chainsep ter -merge all <<-eof
6
1
eof
fi
else
gmx pdb2gmx -f full.pdb -p topol_amber.top -i posre.itp -o full.gro -chainsep ter <<-eof
6
1
eof
fi
...
rm *.mol2 ligand_premature.itp topol.top ligand.gro conf.gro
...
gmx grompp -f ${PATH_TO_SCRIPTS}/mdp/dummy.mdp -c full.gro -r full.gro -p topol_amber.top -o check.tpr -po checkout.mdp -maxwarn 2 || true

### catch potential grompp segfault
if $(tail -1 ${PATH_TO_SLURM}/slurm-${SLURM_JOB_ID}.out | grep -q "core dumped")
then
echo "For ${PDBid}, executing gmx grompp resulted in a segmentation fault/core dump. Deleting folder as this complex should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
rm -r ${OUTPUT_PATH}/${PDBid}
continue
...
for i in $(seq 1 ${numberOfChains}); do
neighbours=$(echo ${neighbours} | sed -e "s/chain $(grep "Protein_chain" topol_amber.top | head -${i} | tail -1 | cut -d "P" -f 2 | cut -d "_" -f 3 | cut -c1)/molecule ${i}/g")
done
else
...
gmx_command="gmx select -s check.tpr -on check.ndx -select ${ligand} ${neighbours}"
...
gmx mindist -f full.gro -s check.tpr -n check.ndx -od check.xvg <<-eof
ligand
modelledResidues
eof
distance=$(tail -1 check.xvg | rev | cut -d " " -f 1 | rev) # distance in nm
distance=$(echo ${distance} 10 | awk '{print $1 * $2}') # distance in A
echo "For ${PDBid}, the minimum distance between the ligand and modelled residues (in A) amounts to: ${distance}"
if (( $(echo "${distance} < 3" | bc -l) ))
then
echo "For ${PDBid}, modelled residues are very close to the bound ligand (less than 3 A). Deleting folder as this protein should not be handled by the current workflow!"
cd ..
rm -r ${PDBid}
rm -r ${OUTPUT_PATH}/${PDBid}
continue
...
## clean up
rm checkout.mdp check.tpr check.ndx check.xvg
...
distance=$(tail -1 check.xvg | rev | cut -d " " -f 1 | rev) # distance in nm
distance=$(echo ${distance} 10 | awk '{print $1 * $2}') # distance in A
echo "For ${PDBid}, the minimum distance between the ligand and modelled atoms (in A) amounts to: ${distance}"
if (( $(echo "${distance} < 3" | bc -l) ))
then
echo "For ${PDBid}, modelled atoms are very close to the bound ligand (less than 3 A). Deleting folder as this protein should not be handled by the current workflow!"
cd ..
rm -r ${PDBid}
rm -r ${OUTPUT_PATH}/${PDBid}
continue
...
for i in $(seq 1 ${numberOfBonds}); do
bond=$(grep -m${i} "bond" CONECT.txt | tail -1)
...
allSG=""
for i in $(seq 1 $(grep "CYS     SG" ${file} | wc -l)); do
allSG=${allSG}" "$(grep "CYS     SG" ${file} | head -${i} | tail -1 | cut -c1-6 | tr -c -d 0-9)
done
...
if $(echo ${bondsTopology} | grep -q " ${i} ${j} ") && (( ${i} != ${j} ))
then
...
rm -r ${OUTPUT_PATH}/${PDBid}
cd ..
cp -r ${PDBid} ${OUTPUT_PATH}
rm -r ${PDBid}
echo "${PDBid} successfully completed!"
