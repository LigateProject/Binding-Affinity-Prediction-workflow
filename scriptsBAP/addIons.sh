...
gmx grompp -f ${PATH_TO_MDP}/em.mdp -c solvated.gro -r solvated.gro -p topol_amber.top -o addIons.tpr -maxwarn 2 || true

## error catching
## addIons.tpr must exist
if ! [ -f addIons.tpr ]
then
echo "For ${PDBid}, ions could not be added to the complex structure (grompp error). Deleting folder as this structure should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
continue
fi
...
set +e
gmx genion -s addIons.tpr -o ions.gro -p topol_amber.top -pname NA -nname CL -neutral <<-eof
SOL
eof
set -e
...
if ! [ -s ions.gro ]
then
echo "For ${PDBid}, ions could not be added to the complex structure (genion error). Deleting folder as this structure should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
continue
fi

rm mdout.mdp addIons.tpr solvated.gro 
if [ -f "#topol_amber.top.1#" ]
then
rm "#topol_amber.top.1#"
echo "For ${PDBid}, ions were added. Removing topology back-up."
else
echo "For ${PDBid}, no ions were added."
fi

echo "For ${PDBid}, the workflow step 'adding ions' has been completed successfully!"
