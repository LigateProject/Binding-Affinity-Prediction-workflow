...
#SBATCH --job-name=BAPPrepareSimulations
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -G 0
#SBATCH --mem=8GB
...
gromppErrors=0
for replicate in $(seq -w 01 08); do
mkdir run_${replicate}
cd run_${replicate}

# run grompp to get input .tpr file
gmx grompp -f ${PATH_TO_MDP}/simulation.mdp -c ../equi.gro -p ../topol_amber.top -n ../indexTraining.ndx -o simulation.tpr -po simulationOut.mdp || true
## error catching
...
if (( ${gromppErrors} == 1 ))
then
echo "For ${PDBid}, at least one of the TPR files could not be generated (grompp error). Deleting folder as this structure should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
continue
fi

echo "For ${PDBid}, all TPR files have been generated successfully."
