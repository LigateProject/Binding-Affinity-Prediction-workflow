...
#PBS -q qgpu_eurohpc
#PBS -N Cont_ASMESIS-SV_Validation
...
cd ${PATH_TO_SIM[${i}]}
# identify the correct CPT file
CPTFile=$(ls -ltr cpt | tail -n 1 | cut -d " " -f 9)
# When is it useful to have separate PME ranks?
# update gpu fails with some weird error message
gmx mdrun -deffnm simulation -pin on -cpnum -cpt 60 -cpo cpt/state -ntmpi ${tMPIranks} -ntomp ${OMPthreads} -pinstride 1 -pinoffset ${offset[${i}]} -gpu_id ${i} -maxh 48 -nb gpu -pme gpu -pmefft gpu -bonded gpu -update cpu -npme 1 -cpi cpt/${CPTFile} -noappend &
PIDs+=($!)
cd ..
