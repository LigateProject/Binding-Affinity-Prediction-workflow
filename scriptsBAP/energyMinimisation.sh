...
#SBATCH --job-name=BAPEnergyMinimisation
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -G 0
#SBATCH --mem=8GB
...
gmx grompp -f ${PATH_TO_MDP}/em.mdp -c ions.gro -r ions.gro -p topol_amber.top -o EM.tpr -po EMout.mdp || true

## error catching
## EM.tpr must exist
if ! [ -f EM.tpr ]
then
echo "For ${PDBid}, the energy minimisation failed (grompp error). Deleting folder as this structure should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
continue
fi
...
if ! [ -s EM.gro ]
then
echo "For ${PDBid}, the energy minimisation failed (mdrun error). Deleting folder as this structure should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
continue
fi
...
echo "For ${PDBid}, the energy minimisation has been completed successfully."
