...
#SBATCH --job-name=BAPcheckProtein
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -G 0
#SBATCH --mem=8GB
...
if [ -f mismatchBetweenSequenceAndStructureSuspected ]
then
cd ..
rm -r ${PDBid}
echo "For ${PDBid}, there seems to be a mismatch between the sequence and the coordinates provided in the PDB file. Please clarify/correct input PDB file! Deleting folder as this protein should not be handled by the current workflow."
continue
fi
...
PDBid2=$(echo "$PDBid" | awk '{print toupper($0)}')
...
if $(tail -${tailNumber} ${PATH_TO_SLURM}/slurm-${SLURM_JOB_ID}.out | head -${ringPunchNumber} | grep -q "filling")
then
gaps=$(tail -${tailNumber} ${PATH_TO_SLURM}/slurm-${SLURM_JOB_ID}.out | head -${ringPunchNumber} | grep -c "filling")
fi
touch missingResidues.txt

for i in $(seq 1 ${gaps}); do
modelled=$(tail -${tailNumber} ${PATH_TO_SLURM}/slurm-${SLURM_JOB_ID}.out | head -${ringPunchNumber} | grep "filling" | tail -${i} | head -1)
...
for j in $(seq $(( ${lowerBound} + 1 )) $(( ${upperBound} - 1 ))); do
if (( ${j} == (${upperBound} - 1) )) && (( ${i} == ${gaps} ))
then
