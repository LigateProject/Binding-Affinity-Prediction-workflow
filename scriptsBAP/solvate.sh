...
gmx editconf -f full.gro -o correctBox.gro -bt dodecahedron -d 1.5 || true

## error catching
## correctBox.gro must exist and must not be empty
if ! [ -s correctBox.gro ]
then
echo "For ${PDBid}, the complex structure could not be placed in a box of the correct size. Deleting folder as this structure should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
continue
fi
...
gmx solvate -cp correctBox.gro -cs spc216.gro -p topol_amber.top -o solvated.gro || true

## error catching
## solvated.gro must exist and must not be empty
if ! [ -s solvated.gro ]
then
echo "For ${PDBid}, the complex structure could not solvated (.gro file not obtained). Deleting folder as this structure should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
continue
...
then
echo "For ${PDBid}, the complex structure could not solvated (water molecules were not added to topology). Deleting folder as this structure should not be handled by the current workflow."
cd ..
rm -r ${PDBid}
continue
fi

rm "#topol_amber.top.1#" full.gro correctBox.gro
sed -i solvated.gro -e "s/HOH/SOL/g"

echo "For ${PDBid}, the complex structure was solvated successfully."
